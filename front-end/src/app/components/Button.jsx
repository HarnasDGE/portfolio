...
export const Button = ({
  ...
  const colors = {
    ...
    link: "text-dark dark:text-light hover:text-primary", // <-- Nie dodajemy wypełnienia tła, tylko kolor tekstu
  };
  ...
  const type =
    color === "link"
      ...
      : isIcon
      ? "p-2 shadow"
      : "px-8 py-4 h-fit shadow";

  const baseClasses = `
    ${colors[color]}
    flex gap-2 w-fit justify-center items-center
    ${type}
    font-semibold  whitespace-nowrap rounded-full ${className}
  `;
